...
for name in reps_name:
    ...
    epoch_losses_train = defaultdict(list)
    epoch_losses_val = defaultdict(list)
    ...
    for seed in seeds:
        ...
        train_dataloader = torch.utils.data.DataLoader(training_dataset, batch_size=4, shuffle=True)
        ...
        for epoch in range(num_epochs):
            ...
            with torch.no_grad():
                ...
                print(f"Epoch: {epoch}, Loss on eval: {losses}")
                epoch_losses_val[epoch].append(losses)
                if losses < best_loss:
                    best_loss = losses
                    # torch.save(model.state_dict(), 'SPARSE_autoencoder_model_FULLmemcat_' + name + '_' + str(seed) + '.pt')
                ...
                print(f"Epoch: {epoch}, Loss on train: {loss_train}\n")
                epoch_losses_train[epoch].append(loss_train)
                ...
                torch.save(model.state_dict(), 'SPARSE_autoencoder_model_FULLmemcat_' + name + '_' + str(seed) + '_' + str(epoch) +  '.pt')
                
    # model corr and losses
    with open('autoencoder_loss_correlation_stats_' + name + '.json', 'w') as f:
        json.dump(epochs_sigs, f)
    	
    with open('autoencoder_epoch_losses_train_' + name + '.json', 'w') as f:
        json.dump(epoch_losses_train, f)

    with open('autoencoder_epoch_losses_val_' + name + '.json', 'w') as f:
        json.dump(epoch_losses_val, f)
